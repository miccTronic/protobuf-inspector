...
protobuf-inspector to help it parse your message better. To do this, create a
`protobuf_config.json` file in the current directory or any parent one, with the
following in it:
...
Let's say we start working with this blob:
    ...
    $ dotnet run --project src/ProtobufInspector -- < my-blob
    root:
...
Our `protobuf_config.json` would turn into:
  ...
  "types": {
    "root": {
      "fields": {
        ...
        "7": { "type": "packed varint" },
        "10": { "type": "float" },
        "14": { "type": "item", "name": "available_items" }
      }
    },
    "item": {
      "fields": {
...
~~~ json
{
  "types": {
    "root": {
      "fields": {
        "1": { "type": "enum request_type", "name": "request" }
      }
    },
    "enum request_type": {
      "enum": {
...
Enum types are defined separately using the `enum` mapping. To use the enum type you
just defined, specify `enum <name>` at a varint field.

## Custom native type aliases

You can register native type aliases to existing handlers via the `native_types`
section. This does not allow custom parsing logic, but it lets you reuse existing
handlers under a clearer name.
...
~~~ json
{
  "types": {
    "root": {
      "fields": {
        "1": { "type": "milliseconds_timestamp", "name": "timestamp" }
      }
    }
  },
  "native_types": {
    "milliseconds_timestamp": "uint64"
  }
}
~~~
